    ...
    <>
      ...
      <div className="bg-white rounded-[2rem] border border-black shadow-sm p-6 sm:p-8 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 animate-[fadeIn_0.4s_ease-out]" >
        ...
        <div className="flex gap-3 flex-wrap">
          <button onClick={() => navigate('/office-incharge/supervision-requests')} className="bg-white text-black hover:bg-white px-5 py-3 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2 cursor-pointer">
            <UserPlus className="w-4 h-4" /> Pending Requests ({stats ? stats.pendingSupervisionReqs : 0})
          </button>
          <button onClick={() => navigate('/office-incharge/grievances')} className="bg-white hover:bg-white text-white px-5 py-3 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2 cursor-pointer">
            <Scale className="w-4 h-4" /> Grievances ({stats ? stats.openGrievances : 0})
          </button>
        </div>
      </div>
