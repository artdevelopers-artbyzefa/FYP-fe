...
import { getSupervisorPhase4Groups, submitSupervisorPhase4Evaluation } from '../../services/phase4.service';
...
const FacultyPhase4Evaluation = () => {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      getSupervisorPhase4Groups(),
      getRubricByPhase('phase4')
    ])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await submitSupervisorPhase4Evaluation({
        groupId: selectedId,
        marks,
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      {groups.length === 0 ? (
        <motion.div variants={item} className="bg-white rounded-2xl border border-line shadow-sm p-12 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">You have no groups to evaluate for Phase 4.</p>
        </motion.div>
      ) : (
        <>
...
export default FacultyPhase4Evaluation;
