...
import { Bell, Calendar, Crown, FileSignature, GitBranch, Landmark, Menu, MessageSquare, Presentation, Star, Tags, X } from 'lucide-react';
            ...
            return (
              <React.Fragment key={link.to}>
                ...
                <NavLink 
                  ...
                  className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 ${isActive ? 'bg-black text-white shadow-lg shadow-blue-600/30 font-bold' : link.highlight ? 'text-black' : 'text-white/80 hover:bg-white/10 hover:text-white'} ${isCollapsed ? 'justify-center w-[44px] h-[44px] mx-auto p-[10px]' : ''}`}
                >
        ...
        <div className="bg-white border-b border-black px-4 md:px-6 h-16 flex items-center justify-between shadow-sm flex-shrink-0 z-[30] gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-base font-black text-black leading-tight truncate">Faculty Dashboard</h2>
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y divide-black">
                    <div className="p-4 hover:bg-white/50 transition-colors flex gap-3 items-start border-l-4 border-black bg-white/20">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-bold text-black">New Project Proposal Submitted</div>
                        <div className="text-[11px] text-black mt-0.5">Group G-042 submitted proposal "AI Traffic Management System" for your review.</div>
                        <div className="text-[10px] text-black mt-1 font-bold">2 hours ago</div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="relative">
              <button className="flex items-center gap-2 p-1 md:px-2.5 md:py-1.5 bg-white rounded-xl cursor-pointer border border-black hover:bg-white hover:border-black transition-all">
                ...
                <div className="text-left hidden sm:block">
                  <div className="text-xs font-bold text-black leading-tight truncate max-w-28">{user.name || 'Dr. Ali Hassan'}</div>
                  <div className="text-[10px] text-black leading-tight font-bold">Assoc. Prof</div>
                </div>
              </button>
            </div>
          </div>
