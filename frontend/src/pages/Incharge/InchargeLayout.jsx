        ...
        <div className="flex items-center gap-3 px-4 py-5 border-b border-line relative">
          ...
          {!sidebarCollapsed && (
            <div className="overflow-hidden transition-all duration-300">
              <div className="text-slate-900 text-sm font-bold whitespace-nowrap tracking-wide">CUI ABBOTTABAD</div>
              <div className="text-slate-400 text-xs whitespace-nowrap leading-tight font-medium">FYP Office In-charge</div>
            </div>
          )}
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1" style={{ scrollbarWidth: 'none' }}>
          {navItems.map((group, idx) => (
            <div key={idx} className="mb-2">
              {!sidebarCollapsed && (
                <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-3 mb-2 pt-2">
                  {group.section}
                </div>
              )}
              {group.items.map((item) => {
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 focus-visible:ring-2 focus-visible:ring-blue-500 ${isActive ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-700'} ${sidebarCollapsed ? 'justify-center' : ''}`}
                    title={item.label}
                  >
