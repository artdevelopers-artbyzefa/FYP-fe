...
export default function DashboardLayout() {
  ...
  const navItems = [
    ...
    { section: 'Group & Supervisor', items: [
      { id: 'group_partners', label: 'FYP Partners', icon: 'fa-users', isDropdown: true, subItems: [
        { id: '/partners/new', label: 'New Request' },
        { id: '/partners/requests', label: 'Incoming Requests' }
      ]},
      { id: '/supervisor-selection', label: 'Supervisor Selection', icon: 'fa-user-tie' }
    ]},
    { section: 'Project Execution', items: [
      { id: 'group_ideas', label: 'Project Idea', icon: 'fa-lightbulb', isDropdown: true, subItems: [
        { id: '/project/new', label: 'New Idea' },
        { id: '/project/approved', label: 'Approved Ideas' }
      ]},
      { id: '/task-manager', label: 'Task Manager', icon: 'fa-tasks' }
    ]}
  ];
  ...
  return (
    <div className={`flex h-screen overflow-hidden relative bg-lightbg selection:bg-secondary/20 ${isLocked ? 'profile-locked' : ''}`} style={{ fontFamily: "'Poppins', sans-serif", backgroundColor: '#EFF6FF' }}>
      
      {/* ═══════════════ SIDEBAR ═══════════════ */}
      <div 
        className={`bg-primary flex flex-col flex-shrink-0 transition-all duration-300 overflow-hidden fixed lg:relative z-[50] h-full ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      ...
      >
              ...
              {group.items.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="mb-1">
                      <div 
                        ...
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 ${isNavLocked ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''} ${isActive && !isOpen ? 'bg-secondary text-white shadow-lg shadow-blue-600/30 font-bold' : 'text-white/90 hover:bg-white/15 hover:text-white'} ${sidebarCollapsed ? 'justify-center' : ''}`}
                        title={item.label}
                      >
                        ...
                        {!sidebarCollapsed && isNavLocked && <i className="fas fa-lock text-[10px] text-white/40 animate-pulse mr-1"></i>}
                        {!sidebarCollapsed && !isNavLocked && <i className={`fas fa-chevron-down text-[10px] transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}></i>}
                      </div>
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 ${isNavLocked ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''} ${isActive ? 'bg-secondary text-white shadow-lg shadow-blue-600/30 font-bold' : 'text-white/90 hover:bg-white/15 hover:text-white'} ${sidebarCollapsed ? 'justify-center' : ''}`}
                    title={item.label}
                  >
                    <i className={`fas ${item.icon} text-sm w-5 text-center flex-shrink-0`}></i>
                    {!sidebarCollapsed && <span className="text-sm font-medium flex-1 whitespace-nowrap">{item.label}</span>}
                    {!sidebarCollapsed && isNavLocked && <i className="fas fa-lock text-[10px] text-white/40 animate-pulse"></i>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="p-2 border-t border-white/10">
          <div onClick={() => logout()} className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer text-white/90 hover:bg-red-500/20 hover:text-red-400 transition-all duration-200" title="Logout">
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full transition-all duration-300 ease-in-out">
        ...
        <div className="bg-white border-b border-gray-100 px-3 md:px-6 h-14 md:h-16 flex items-center justify-between shadow-sm flex-shrink-0 z-[30] gap-3">
          <div className="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="hidden lg:flex w-9 h-9 rounded-xl bg-gray-50 items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-primary transition-all border-0 flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
            ...
            <div className="relative">
              <button className="w-9 h-9 rounded-xl bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-blue-50 hover:text-secondary transition-all">
                <i className="fas fa-bell text-sm"></i>
                <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white"></span>
              </button>
            </div>

            <Link to="/profile" className="flex items-center gap-2 p-1 md:px-2.5 md:py-1.5 bg-gray-50 rounded-xl cursor-pointer border border-gray-100 hover:bg-blue-50 hover:border-blue-100 transition-all">
              <div className="w-8 h-8 bg-blue-100/50 rounded-lg flex items-center justify-center text-primary text-xs font-bold shadow-sm flex-shrink-0">
                {user.avatar || 'ST'}
              </div>
