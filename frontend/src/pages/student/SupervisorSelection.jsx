  ...
  return (
    <div className="animate-in fade-in slide-in- duration-300 max-w-3xl mx-auto space-y-6">
      <div className="bg-white rounded-2xl border border-black shadow-sm p-6">
        <h2 className="text-xl font-bold text-black mb-2">Supervisor Selection</h2>
          ...
          <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm font-bold text-gray-800 mt-0.5">{group.supervisor.name}</div>
                <div className="text-xs text-gray-500">{group.supervisor.email}</div>
              </div>
        ...
        {!hasSupervisor && !isPending && (
          <>
            <div className="flex items-center gap-2 bg-white border border-black rounded-xl px-4 py-2.5 focus-within:border-blue-600 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
              ...
              {query && !searchLoading && (
                <button onClick={() => { setQuery(''); doSearch(''); }} className="bg-transparent border-0 p-0 text-gray-400 hover:text-gray-600 cursor-pointer">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
            ...
            <div className="mt-4 border border-gray-200 rounded-xl overflow-hidden">
              {results.length === 0 ? (
                ...
                results.map(sup => (
                  <div key={sup.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 border-b border-gray-50 last:border-0 transition-colors">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        <div className="font-bold text-sm text-gray-800 truncate">{sup.name}</div>
                        <div className="text-xs text-gray-500 mt-0.5 truncate">{sup.email} {sup.tags?.length > 0 ? `• ${sup.tags.join(', ')}` : ''}</div>
                      </div>
                    </div>
                    <button
