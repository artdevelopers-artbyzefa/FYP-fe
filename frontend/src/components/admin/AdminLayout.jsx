        ...
        <div className="flex items-center gap-3 px-4 py-5 border-b border-line flex-shrink-0">
          ...
          {!sidebarCollapsed && (
            <div className="overflow-hidden">
              ...
              <div className="text-slate-400 text-xs whitespace-nowrap leading-tight">System Administrator</div>
            </div>
          )}
          ...
          {navItems.map((item) => (
            <React.Fragment key={item.id}>
              {item.section && !sidebarCollapsed && (
                <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-3 mb-1 pt-3">{item.section}</div>
              )}
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 text-left outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2
                  ${location.pathname === item.id ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-700'}
