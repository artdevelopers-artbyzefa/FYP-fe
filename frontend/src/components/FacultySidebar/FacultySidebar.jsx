...
const FacultySidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar container */}
      <div className={`w-[260px] bg-white h-screen text-slate-900 flex flex-col p-6 fixed left-0 top-0 border-r border-line shadow-card z-50 transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        {/* Brand Logo */}
        <div className="flex items-center gap-3.5 mb-8 px-2 shrink-0">
          <div className="w-11 h-11 bg-blue-100 rounded-2xl flex items-center justify-center text-blue-600 shadow-sm shrink-0">
            <UniversityIcon className="w-6 h-6" />
          </div>
          <div className="flex flex-col">
            <span className="text-slate-900 text-sm font-bold tracking-tight leading-none font-poppins">CUI ABBOTTABAD</span>
            <span className="text-slate-400 text-xs font-medium mt-1.5 font-poppins">Faculty Supervisor</span>
          </div>
        </div>
        ...
        <div className="pt-6 mt-6 border-t border-line shrink-0">
          <button 
            onClick={() => {
              handleLinkClick();
              handleLogout();
            }}
            className="w-full flex items-center gap-3.5 px-4 py-3 rounded-[16px] text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all duration-200 font-poppins font-semibold text-[15px]"
          >
