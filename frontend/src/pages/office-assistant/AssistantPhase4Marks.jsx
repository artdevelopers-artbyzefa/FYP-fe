...
import { getPhase4Marks } from '../../services/phase4.service';
import { FileText, Printer, Search } from 'lucide-react';

const AssistantPhase4Marks = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-line pb-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Phase 4 (100%) Marks</h2>
      ...
      {results.length > 0 && (
        <div className="relative max-w-xs">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, group, reg no…"
      ...
      <div className="bg-white rounded-2xl border border-line shadow-sm overflow-hidden" id="printArea">
        <div className="hidden print:block p-6 text-center border-b border-line">
          ...
          <h2 className="text-sm font-bold text-slate-700 mt-1">FYP Phase 4 (100%) Final Evaluation</h2>
        </div>
                ...
                filtered.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.id || idx} className="hover:bg-slate-50 transition-colors">
                      ...
                      {[0, 1, 2, 3].map(i => <td key={i} className="py-4 px-6 text-center font-bold text-slate-900 text-xs">{cm[i] ?? '—'}</td>)}
                        ...
                        <div className="relative group inline-block">
                          ...
                          {remarks.length > 0 && (
                            <div className="absolute right-0 top-7 w-64 bg-white border border-line rounded-xl shadow-dropdown z-10 hidden group-hover:block text-left">
                              <div className="px-3 py-2 border-b border-line bg-slate-50 rounded-t-xl"><span className="text-[10px] font-bold text-slate-700">Remarks</span></div>
                              <div className="max-h-40 overflow-y-auto divide-y divide-line">
                                {remarks.map((rem, i) => (
                                  <div key={i} className="px-3 py-2"><span className="text-[9px] font-bold text-blue-600 block mb-0.5">{rem.who}</span><p className="text-[10px] text-slate-600 leading-relaxed">{rem.text}</p></div>
                                ))}
                              </div>
                            </div>
                          )}
...
const SkeletonTable = () => (
  <div className="bg-white rounded-2xl border border-line shadow-sm overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-50 border-b border-line">
    {[1,2,3,4,5,6,7,8,9].map(i=><th key={i} className="py-3.5 px-6"><div className="skeleton h-4 rounded-md" style={{width:i<2?'100px':'55px'}}/></th>)}
  </tr></thead><tbody className="divide-y divide-line">
    {[1,2,3,4,5].map(i=><tr key={i} className="animate-pulse">{[1,2,3,4,5,6,7,8,9].map(j=><td key={j} className="py-4 px-6"><div className="skeleton h-4 rounded-md" style={{width:j<2?'120px':'45px'}}/></td>)}</tr>)}
  </tbody></table></div></div>
);

export default AssistantPhase4Marks;
