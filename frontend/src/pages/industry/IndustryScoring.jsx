...
export default function IndustryScoring() {
  ...
  if (!selectedGroup) return <div className="p-8 text-center text-slate-900 font-bold">Please select a project from the Dashboard first.</div>;
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pb-6 border-b border-line gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="font-black text-slate-900 text-lg">Group {selectedGroup.groupId}: {selectedGroup.title}</span>
        ...
        {scoringLocked ? (
          ...
          <form onSubmit={handleScorecardSubmit} className="space-y-6">
            <div className="space-y-4">
              <span className="text-xs font-bold text-slate-900 uppercase tracking-wider block mb-2">External Criteria Scorecard Input</span>
              {[
            ...
            <div className="pt-4 border-t border-line flex justify-between items-center gap-3">
              <button type="button" onClick={() => navigate('/industry-dashboard')} className="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-900 hover:bg-white transition-colors cursor-pointer">
                ← Back to Projects
              </button>
              <button type="submit" className="text-white px-8 py-3 rounded-xl font-bold text-sm shadow-lg transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none flex items-center gap-2" style={{ backgroundColor: '#2563EB' }}>
