...
export default function FYPGroup() {
  ...
  if (loadingData) return <div className="flex items-center justify-center min-h-[60vh]"><Loader className="animate-spin text-black text-3xl" /></div>;
  ...
  return (
    <div className="animate-in fade-in slide-in- duration-300 max-w-3xl mx-auto space-y-6">
      <div className="bg-white rounded-2xl border border-black shadow-sm p-6">
        <h2 className="text-xl font-bold text-black mb-2">FYP Group</h2>
        ...
        {group && group.members?.length > 0 && (
          <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Current Members ({group.members.length}/3)</h3>
            <div className="space-y-2">
              {group.members.map((m, i) => (
                <div key={m._id || i} className="flex items-center gap-3 px-3 py-2 bg-white rounded-lg border border-gray-100">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-bold text-gray-800 truncate">{m.user?.name || 'Unknown'}{i === 0 ? ' (Leader)' : ''}</div>
                    <div className="text-xs text-gray-500">{m.regNo || ''}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {!isFull && (
          <>
            <div className="flex items-center gap-2 bg-white border border-black rounded-xl px-4 py-2.5 focus-within:border-blue-600 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
              ...
              {searchLoading && <Loader className="w-4 h-4 animate-spin text-gray-400 flex-shrink-0" />}
              {query && !searchLoading && (
                <button onClick={() => { setQuery(''); setResults([]); setShowResults(false); }} className="bg-transparent border-0 p-0 text-gray-400 hover:text-gray-600 cursor-pointer">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
                ...
                {results.length === 0 ? (
                  ...
                  results.map(student => (
                    <div key={student.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 border-b border-gray-50 last:border-0 transition-colors">
                      ...
                      <button disabled={sending[student.id]} onClick={() => handleSend(student.id)} className="ml-3 flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-2 rounded-lg text-xs font-bold transition-all border-0 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5">
      ...
      {incomingRequests.length > 0 && !isFull && (
        <div className="bg-white rounded-2xl border border-black shadow-sm p-6">
          <h3 className="text-sm font-bold text-gray-700 mb-4">Incoming Requests ({incomingRequests.length})</h3>
          <div className="space-y-3">
            {incomingRequests.map(r => (
              <div key={r.id} className="flex items-center justify-between px-4 py-3 border border-gray-100 rounded-xl bg-gray-50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gray-200 text-gray-600 rounded-full flex items-center justify-center font-bold text-sm">{r.name.substring(0,2).toUpperCase()}</div>
                  <div>
                    <div className="text-sm font-bold text-gray-800">{r.name}</div>
                    <div className="text-xs text-gray-500">{r.regNo} {r.cgpa ? `• CGPA: ${r.cgpa}` : ''}</div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleRespond(r.id, 'accepted')} className="bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-2 rounded-lg text-xs font-bold transition-all border-0 cursor-pointer flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> Accept</button>
                  <button onClick={() => handleRespond(r.id, 'rejected')} className="bg-red-500 hover:bg-red-600 text-white px-3.5 py-2 rounded-lg text-xs font-bold transition-all border-0 cursor-pointer flex items-center gap-1.5"><X className="w-3.5 h-3.5" /> Reject</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {sentRequests.length > 0 && (
        <div className="bg-white rounded-2xl border border-black shadow-sm p-6">
          <h3 className="text-sm font-bold text-gray-700 mb-4">Sent Requests ({sentRequests.length})</h3>
          <div className="space-y-2">
            {sentRequests.map(r => (
              <div key={r.id} className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-xl border border-gray-100">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-gray-200 text-gray-600 rounded-full flex items-center justify-center font-bold text-xs">{r.name.substring(0,2).toUpperCase()}</div>
                  <div>
                    <div className="text-sm font-bold text-gray-800">{r.name}</div>
