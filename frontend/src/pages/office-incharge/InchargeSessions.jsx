        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-black shadow-sm p-6">
          ...
          <form onSubmit={handleSessionSubmit} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div><label className="block text-xs font-bold text-black mb-1.5">Session Name</label><input type="text" defaultValue={sessionData?.sessionName || "Spring 2026 (SP26)"} className="w-full bg-white border border-black rounded-xl px-4 py-2.5 text-sm font-bold text-black outline-none focus:border-black" required /></div>
              <div><label className="block text-xs font-bold text-black mb-1.5">Session Duration</label><input type="text" defaultValue={sessionData?.duration || "Feb 01, 2026 – Jul 15, 2026"} className="w-full bg-white border border-black rounded-xl px-4 py-2.5 text-sm font-bold text-black outline-none focus:border-black" required /></div>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div><label className="block text-[10px] font-bold text-black mb-1">Final Defense (100%)</label><input type="date" defaultValue="2026-07-05" className="w-full bg-white border border-black rounded-xl px-3 py-2 text-xs font-bold text-black outline-none focus:border-black cursor-pointer" required /></div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-blue-600 text-xs font-bold text-black">
                  {sessionData?.repeats?.map((r, idx) => (
                    <tr key={idx} className="hover:bg-white/50">
                      ...
                      <td className="py-3 px-4 text-black font-mono">{r.regNo}</td>
                      <td className="py-3 px-4"><span className="bg-white text-black px-2 py-0.5 rounded-lg border border-black">{r.status}</span></td>
                      <td className="py-3 px-4 text-right space-x-1">
                        <button onClick={handleClear} className="px-2.5 py-1 rounded-lg bg-white text-black hover:bg-white border border-black transition-all cursor-pointer">Clear</button>
                        <button onClick={handleEnroll} className="px-2.5 py-1 rounded-lg bg-white text-black hover:bg-white border border-black transition-all cursor-pointer">Enroll</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
