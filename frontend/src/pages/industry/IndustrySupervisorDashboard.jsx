...
export default function IndustrySupervisorDashboard() {
  ...
  /* ── Nav items ──────────────────────────────────────────────────────────── */
  const navItems = [
    { id: 'dashboard', label: 'Assigned Projects', icon: 'fa-project-diagram', section: 'Evaluation Console' },
    { id: 'scoring',   label: 'Rubric Scoring',    icon: 'fa-star-half-alt',   section: null },
  ];
        ...
        <header className="bg-white border-b border-gray-100 px-4 md:px-6 h-16 flex items-center justify-between shadow-sm flex-shrink-0 z-30 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <button
              ...
              className="hidden lg:flex w-9 h-9 rounded-xl bg-gray-50 items-center justify-center text-gray-500 hover:bg-gray-100 transition-all border-0 flex-shrink-0 cursor-pointer"
            >
                      ...
                      <tbody className="divide-y divide-gray-100 text-sm font-medium text-gray-700">
                        {projects.map(p => (
                          <tr key={p.groupId} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="py-4 px-6">
                              <button onClick={() => toast.info(`Downloading ${p.thesisFile}...`)} className="text-xs font-bold text-blue-600 hover:underline">
                  ...
                  {scoringLocked ? (
                    ...
                    <form onSubmit={handleScorecardSubmit} className="space-y-6">
                      ...
                      <div className="pt-4 border-t border-gray-100 flex justify-between items-center gap-3">
                        <button type="button" onClick={() => goTo('dashboard', null)} className="px-5 py-2.5 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-50 transition-colors cursor-pointer">
                          ← Back to Projects
                        </button>
                        <button
                          type="submit"
